...
| `traditional/` | Classic OCR pipeline (no VLM) — CV preprocess → layout/structure → OCR → XY-cut reading order → Markdown | PaddleOCR **PP-OCRv4** + **PP-StructureV2** (tables→HTML), **CPU** (Blackwell GPU broken with paddle 3.3 → 0 det), full-page OCR fallback | ✅ **baseline 100-img**: text_block Edit_dist **0.342** (beats hybrid 0.467), reading_order **0.367**, table TEDS 0.170, **5.06s/img** (5.7× faster than hybrid). 31/31 tests. Runs in `traditional/results/runs/` | [`traditional/AGENT.md`](traditional/AGENT.md) |
